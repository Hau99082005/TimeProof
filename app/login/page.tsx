    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-blue-50 dark:bg-black">
      <main className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">Nhập thông tin để tiếp tục</p>
        </div>
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-xs text-slate-600 dark:text-slate-400">hoặc đăng nhập bằng email</span>
          <div className="flex-1 h-px bg-slate-200 dark:bg-slate-700"></div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-blue-100 dark:border-slate-800">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label htmlFor="password" className="text-xs font-medium text-slate-700 dark:text-slate-300">Mật khẩu</label>
              <a href="#" className="text-xs text-blue-600 dark:text-blue-400 font-medium hover:text-blue-700 dark:hover:text-blue-300 transition-colors">Quên mật khẩu?</a>
            </div>
            <div className="relative">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white py-3 rounded-xl text-sm font-semibold transition-all duration-200 hover:shadow-xl hover:-translate-y-1 active:translate-y-0 mt-2"
          >
            {isLoading ? (
              <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
            ) : (
              <><span>Đăng nhập</span><ArrowRight size={16}></ArrowRight></>
            )}
          </button>
        </form>
        ...
        <p className="text-center text-sm text-slate-600 dark:text-slate-400 mt-6">
          Chưa có tài khoản?
          <Link href="/register" className="text-blue-600 dark:text-blue-400 font-medium hover:text-blue-700 dark:hover:text-blue-300 transition-colors ml-1">Đăng ký ngay</Link>
        </p>
      </main>
    </div>
  );
